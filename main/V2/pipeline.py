...
Still single-source (Tavily) until YouTube keys are added.
...
def node_fetch_show_metadata(state: RecapState) -> dict:
    edition, season = state["edition"], state["season"]
    imdb_id = load_imdb_id(edition)
    if not imdb_id:
        print(f"[omdb] no hardcoded IMDb ID for {edition} season {season}, skipping metadata fetch")
        return {"episode_titles": {}}

    key = os.getenv("OMDB_API_KEY") or os.getenv("OMBD_API_KEY")
    if not key:
        print("[omdb] no OMDB_API_KEY/OMBD_API_KEY found, skipping metadata fetch")
        return {"episode_titles": {}}

    try:
        response = requests.get("http://www.omdbapi.com/", params={"apikey": key, "i": imdb_id, "Season": season}, timeout=10)
        data = response.json()
    except requests.RequestException as e:
        print(f"[omdb] request failed ({e}), continuing without metadata")
        return {"episode_titles": {}}

    if data.get("Response") != "True":
        print(f"[omdb] lookup failed: {data.get('Error')}, continuing without metadata")
        return {"episode_titles": {}}
    ...
    return {"episode_titles": episode_titles}
