...
**Project**: "Previously On: Love Is Blind" — autonomous, spoiler-bounded recap generator.

**Problem statement**: As a Love Is Blind viewer who watches week by week (or catches up later), I forget prior drama between viewings, and I want a quick, readable recap of what happened in the season I'm watching, right before an episode or between watch sessions. 

**Existing solutions**: Existing recap content (YouTube videos, articles) is too long, inconsistent in style and coverage across seasons and editions, and doesn't reliably match a specific viewer's progress, creating either spoilers or an information gap.
...
**Verify**:
- MVP level: recap is tonally consistent and recognizably comic/dramatic across repeated runs, correctly bounded to the user's cutoff (zero spoilers past it), returns structured output.
...
**Current process**: Informally asking other viewers, occasionally checking a YouTube video. Coverage and tone vary by creator, season, and edition, and there is no reliable way to get a recap bounded to the user's own progress.
...
**Competitors and competitive advantage**: Amazon Prime Video ships a native equivalent (X-Ray Recaps, spoiler-free text summaries; Video Recaps, AI-narrated video recaps), but it's Prime-only,limited to select scripted Prime Originals, and tonally neutral. My product edge is a comic narrator voice, inclusion of fan reaction alongside plot facts specifically for the show Love is Blind.


**User story**:
As a Love Is Blind viewer, I want to tell my edition, season, and the last episode I watched, so that I get a recap tailored exactly to where I've stopped, without spoilers or hunting through inconsistent YouTube videos and articles.
...
- Given I explicitly ask for "After the show", when I get my recap, then I see where the couples stand today.
---
...
- Cumulative recap: covers everything up to and including the user's requested episode, not the episode in isolation.
...
|---|---|---|---|---|
| V0 (MVP) | Prove summary is viable | A good, on-tone, spoiler-bounded recap can be generated at all | Manual review against MVP success metrics: tone, zero leakage, structured fields, source accuracy | Single source (web search), hardcoded show/season, cumulative recap, structured output |
...
| V2 | Prove more sources help | Adding sources and RAG measurably improves recap quality over single source | A/B comparison of V0/V1 output vs. V2 output, source coverage accuracy metric | Add TMDB and YouTube transcripts (3 MCP tools total), Pinecone/RAG with phase tagging, external friend test |
...
- 100% of generated recaps include a valid source list.
- Friend tester rates the recap's tone as "clearly comic/dramatic" (not neutral) on a majority of test reads.
